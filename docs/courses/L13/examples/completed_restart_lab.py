"""L13 real process restart experiment; real Eval, teaching review fields, no human acceptance."""
...
def audit(runtime: Path) -> dict:
    ...
    evidence = {"basis": "maintainer_process_interruption", "student_achievement": False,
                "real_codex_execution": False, "human_acceptance": False, "runtime": str(runtime)}
    ...
    try:
        ...
        code, completed = request(port, "POST", f"/api/v1/tasks/{task_id}/review",
                                  {"reviewer": "teaching-review-field", "decision": "approve",
                                   "note": "Teaching field only; not authenticated human acceptance."})
        evidence["teaching_review_response"] = {"http": code, "body": completed}
        assert code == 200
        code, completed = request(port, "GET", accepted["status_url"])
        assert code == 200 and completed["status"] == "completed"
        evidence["completed_before_restart"] = completed
        second.terminate()
        second.wait(timeout=10)
        evidence["completed_service_exit"] = {"pid": second.pid, "returncode": second.returncode}
        third, port = launch("third")
        code, restored = request(port, "GET", accepted["status_url"])
        assert code == 200 and restored == completed
        assert third.pid != second.pid
        evidence["completed_after_restart"] = restored
        evidence["restarted_service"] = {"pid": third.pid, "http": code}
